...
export const router = createRouter({
  ...
  scrollBehavior(to, _from, savedPosition) {
    ...
    if ((to.name === 'story' || to.name === 'work') && id) {
      ...
      // Mobile (§1f/§1g): land on the snap area; `top` = sticky chrome above it (topbar 48 + the
      // 40px PART header on /story), and the document's scroll-snap seats it exactly. R91: the panels
      // are lazy chunks, so the snap area may not exist yet when this runs; wait for it (≤ 3 s).
